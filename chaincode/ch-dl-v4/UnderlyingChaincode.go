...
func (c *UnderlyingChaincode) Invoke(stub shim.ChaincodeStubInterface) pb.Response {
	...
	case uploadAsset_v1:
		resp = c.uploadAssetv1(stub, args)
	case updateAsset:
	...
	case getAssetList:
		...
		// -- 对数据进行授权
	case setSignByData:
		resp = c.setSignByData(stub, args)

	case getHistory:
		resp = c.getHistory(stub, args)
	default:
...
func (c *UnderlyingChaincode) uploadAsset(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	...
	//assetType := args[0]
	// 获取上链结构体
	assetStruct, err := getAssetStructByType(KYC)
	if err != nil {
		return shim.Error(err.Error())
	}
	// 资产的上链
	...
	// 资产关联  权限  数据
	if _, err := uploadAssetInternalMaster(stub, args, assetStructrel); err != nil {
		return shim.Error(err.Error())
	}

	//	外键
	err = updateAssociation(stub, args)
...
//	新增|| 数据上链
func (c *UnderlyingChaincode) uploadAssetv1(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	// 0　　资产类型
	// 1　　资产结构体json
	// 2　　标签结构体
	// 参数检查
	...
	// 资产关联  权限  数据
	if _, err := uploadAssetInternalRel(stub, args, assetStructrel); err != nil {
		return shim.Error(err.Error())
	}
...
// -- 权限  授权
func (c *UnderlyingChaincode) setSignByData(stub shim.ChaincodeStubInterface, args []string) pb.Response {

	err := SignByData(stub, args)
	//
	if err != nil {
		return shim.Error(err.Error())
	} else {
...
//	查询历史记录
func (c *UnderlyingChaincode) getHistory(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	...
	BytetList, err := getHistoryForKeyStrcts(stub, currentAssetID)
	if err != nil {
		return shim.Error(err.Error())
	}
	return shim.Success(BytetList)
}
...
func getHistoryForKeyStrcts(stub shim.ChaincodeStubInterface, queryString string) ([]byte, error) {
	...
	for resultsIterator.HasNext() {
		...
		row.Value = string(queryResponse.Value)
